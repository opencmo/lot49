...
import com.amazonaws.services.dynamodbv2.nio.http.AmazonHttpClient;
 ...
 * <p>
 * Responsible for basic client capabilities that are the same across all AWS SDK Java clients (ex:
 * setting the client endpoint).
 */
public abstract class AmazonWebserviceClient {
    ...
    static {
        ...
        // rest of the library.
        boolean success = com.amazonaws.log.InternalLogFactory
                        .configureFactory(new CommonsLogFactory());
     ...
     * <p>
     * Subclass should only read but not assign to this field, at least not without synchronization
     * on the enclosing object for thread-safety reason.
     */
     ...
     * Returns the signer.
     * <p>
     * Note, however, the signer configured for S3 is incomplete at this stage as the information on
     * the S3 bucket and key is not yet known.
     */
     ...
     * default protocol from this client's {@link com.amazonaws.ClientConfiguration} will be used,
    ...
    private URI toURI(String endpoint) throws IllegalArgumentException {
        return toUri(endpoint, clientConfiguration);
    }
    ...
    /**
     * An internal method that is not expected to be normally called except for AWS internal
     * development purposes.
     * <p>
     * Overrides the default endpoint for this client ("http://dynamodb.us-east-1.amazonaws.com/")
     * and explicitly provides an AWS region ID and AWS service name to use when the client
     * calculates a signature for requests. In almost all cases, this region ID and service name are
     * automatically determined from the endpoint, and callers should use the simpler one-argument
     * form of setEndpoint instead of this method.
     * <p>
    ...
    /**
     * @deprecated this method is now a no-op, as overriding the signer from sublcass is no longer
     *             supported.
     */
    @Deprecated
     ...
     * <p>
     * Note, however, the signer returned for S3 is incomplete at this stage as the information on
     * the S3 bucket and key is not yet known.
     */
     ...
     *            directly for configuring the signer.
     */
    private Signer computeSignerByServiceRegion(String serviceName, String regionId,
                    String signerRegionOverride, boolean isRegionIdAsSignerParam) {
     ...
     *             region. See {@link com.amazonaws.regions.Region#isServiceSupported(String)}
    ...
    public void setRegion(Region region) throws IllegalArgumentException {
        ...
        URI uri = new DefaultServiceEndpointBuilder(serviceNameForEndpoint,
                        clientConfiguration.getProtocol().toString()).withRegion(region)
                                        .getServiceEndpoint();
        Signer signer = computeSignerByServiceRegion(serviceNameForSigner, region.getName(),
                        signerRegionOverride, false);
        synchronized (this) {
            this.endpoint = uri;
            this.signer = signer;
        }
    ...
    /**
     * @deprecated by {@link #addRequestHandler(com.amazonaws.handlers.RequestHandler2)}.
     *
    ...
    /**
     * Returns the client specific {@link com.amazonaws.metrics.RequestMetricCollector}; or null if
     * there is none.
     */
     ...
     *         returns the value of the regionMetadataServiceName configuration in the internal
     *         config file if such configuration is specified for the current client, otherwise it
     *         returns the same service name that is used for request signing.
     */
     ...
     *
     * @param request
     *            the request passed in from the user
     ...
     * metrics. Caller of this routine is responsible for starting the event for
     * {@link com.amazonaws.util.AWSRequestMetrics.Field#ClientExecuteTime} and call this method in
     * a try-finally block.
     *
     * @param loggingAwsRequestMetrics
