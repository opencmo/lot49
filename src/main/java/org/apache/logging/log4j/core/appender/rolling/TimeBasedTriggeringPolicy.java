...
public final class TimeBasedTriggeringPolicy implements TriggeringPolicy {
    ...
    public void initialize(final RollingFileManager aManager) {
        ...
        // LOG4J2-531: call getNextTime twice to force initialization of both prevFileTime and
        // nextFileTime
        aManager.getPatternProcessor().getNextTime(aManager.getFileTime(), interval, modulate);

        nextRolloverMillis = aManager.getPatternProcessor().getNextTime(aManager.getFileTime(),
                        interval, modulate);
    }
    ...
    public boolean isTriggeringEvent(final LogEvent event) {
        ...
        if (nowMillis >= nextRolloverMillis) { // >
            nextRolloverMillis = manager.getPatternProcessor().getNextTime(nextRolloverMillis,
                            interval, modulate); // nowMillis
            return true;
        }
        return false;
    }
     ...
     * @param modulate
     *            If true the time will be rounded to occur on a boundary aligned with the
     *            increment.
    ...
    public String toString() {
        return "TimeBasedTriggeringPolicy(nextRolloverMillis=" + nextRolloverMillis + ", interval="
                        + interval + ", modulate=" + modulate + ")";
    }
