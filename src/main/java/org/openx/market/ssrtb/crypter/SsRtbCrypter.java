...
/**
 * A class that can encrypt and decrypt the price macro used in Server-Side Real-time bidders.
 */
public class SsRtbCrypter {
    ...
    public long decodeDecrypt(String base64Websafe, String b64EncryptKey, String b64IntegrityKey)
                    throws SsRtbDecryptingException {
        return decodeDecrypt(base64Websafe, b64StrToKey(b64EncryptKey),
                        b64StrToKey(b64IntegrityKey));
    }
    ...
    public long decodeDecrypt(String base64Websafe, SecretKey encryptKey, SecretKey integrityKey)
                    throws SsRtbDecryptingException {
    ...
    public byte[] decrypt(byte[] ciphered, String b64EncryptKey, String b64IntegrityKey)
                    throws SsRtbDecryptingException {
        return decrypt(ciphered, b64StrToKey(b64EncryptKey), b64StrToKey(b64IntegrityKey));
    }

    public byte[] encrypt(byte[] unciphered, SecretKey encryptionKey, SecretKey integrityKey) {
        ...
        outBuff.putLong(BidderCalendar.getInstance().currentTimeMillis());
    ...
    /**
     * Converts a long to an 8 byte array. Code is derived from that used in
     * {@link java.io.DataOutputStream#writeLong(long)}.
     * 
     * @param val
    ...
    /**
     * Converts an array of 8 bytes into a long. Code is dervied from that used in
     * {@link java.io.DataInputStream#readLong()}.
     * 
     * @param buff
     * @return value of the 8 byte array as a long
     */
    protected long toLong(byte[] buff) {
        return (((long) buff[0] << 56) + ((long) (buff[1] & 255) << 48)
                        + ((long) (buff[2] & 255) << 40) + ((long) (buff[3] & 255) << 32)
                        + ((long) (buff[4] & 255) << 24) + ((buff[5] & 255) << 16)
                        + ((buff[6] & 255) << 8) + ((buff[7] & 255) << 0));
    }
}
