...
public class AuctionsSvcSpec_onLiverail {
    ...
    public void setUp() throws Throwable {
        ...
        lAdapterMock = PowerMockito.mock(LiverailAdapter.class);
        Mockito.when(lAdapterMock.convertRequest(any(OpenRtbRequest.class))).thenCallRealMethod();

        PowerMockito.whenNew(LiverailAdapter.class).withNoArguments().thenReturn(lAdapterMock);
    ...
    @Test
    public void positiveFlow_shouldConfiureAdapterAndCallOnBidRequestDelegate() throws Throwable {


        svc.onLiverail(Mockito.mock(AsyncResponse.class), Mockito.mock(UriInfo.class),
                        new OpenRtbRequest(), "xff", Mockito.mock(HttpServletRequest.class), "xrip",
                        "debug");

        // verify adapter constructor call
        PowerMockito.verifyNew(LiverailAdapter.class).withNoArguments();

        // verify adapter call to convertRequest
        Mockito.verify(lAdapterMock, times(1)).convertRequest(any(OpenRtbRequest.class));
    ...
    public void negativeFlow_shouldLogBrockenRequest() throws Exception {
        ...
        svc.onLiverail(Mockito.mock(AsyncResponse.class), Mockito.mock(UriInfo.class), badReq,
        ...
        // verify exception
        PowerMockito.verifyStatic(Mockito.times(1));
        LogUtils.error(Mockito.eq("Error parsing request from LiveRail: null"),
